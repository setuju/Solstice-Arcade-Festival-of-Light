  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
        <div className="bg-gradient-to-br from-indigo-950 to-purple-950 p-8 rounded-2xl border-2 border-yellow-500 shadow-2xl max-w-sm w-full text-center">
            ...
            <h2 className="text-2xl font-black text-white mb-2">ACHIEVEMENT UNLOCKED</h2>
            <h3 className="text-xl font-bold text-yellow-400 mb-4">{title}</h3>
            <p className="text-gray-300 mb-8">{description}</p>
            <button onClick={onClose} className="w-full py-3 bg-yellow-500 text-black font-bold uppercase tracking-widest rounded shadow-lg hover:bg-yellow-400 transition-colors">Amazing!</button>
        </div>
    </div>
  );
}
